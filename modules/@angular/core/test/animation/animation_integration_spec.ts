...
import {animation, transition, style, animate, group, sequence, AnimationEntryMetadata} from '../../src/animation/metadata';
                ...
                fakeAsync(
                    (tcb: TestComponentBuilder, driver: MockAnimationDriver, zone: MockNgZone) => {
                      ...
                      makeAnimationCmp(tcb, '<div *ngIf="exp"></div>', 'ngLeave',
                                       [style({'width': 100}), animate(500, [style({'width': 0})])],
                                       (fixture) => {
                ...
                fakeAsync(
                    (tcb: TestComponentBuilder, driver: MockAnimationDriver, zone: MockNgZone) => {
                      tcb.overrideAnimations(
                             DummyIfCmp,
                             {
                               "ngEnter": [style({'width': 0}), animate(1000, [style({'width': 100})])],
                               "ngLeave": [style({'height': 100}), animate(1000, [style({'height': 0})])]
                             })
                          .createAsync(DummyIfCmp)
                          .then((fixture) => {
