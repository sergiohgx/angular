...
import {removeEventListener} from 'angular2/src/animate/util';
...
export class CssAnimationsDriver implements AnimationDriver {
  ...
  private _animate(element, styles: {[key: string]: string}, options: any): AnimationPlayer {
    var existingStyle = DOM.getStyle(element, 'transition');
    DOM.setStyle(element, 'transition', normalizeCssTime(options['duration']) + ' linear all');
    this.style(element, styles);
    var player = new CssAnimationsPlayer(element, options);
    player.subscribe(() => {
  ...
  animateFromTo(element: HTMLElement,
    ...
    return this._animate(element, endStyles, options);
  }
}
