...
export class Animation {
  ...
  setup(): void {
    ...
    if (this.data.duration != null) {
      this._temporaryStyles['transitionDuration'] = this._readStyle('transitionDelay');
    ...
    if (!StringMapWrapper.isEmpty(this.data.animationStyles)) {
      // it's important at we setup a list of the styles and their
  ...
  private _formatStyleProp(prop: string): string {
    ...
    var reg = RegExpWrapper.create('animation');
    return RegExpWrapper.test(reg, prop) ? this._stringPrefix + prop : prop;
  }
